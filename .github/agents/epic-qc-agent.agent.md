...
2. Confirm whether `/src` changed in the delivered work.
3. If `/src` did not change, report that QC correctly skipped test execution.
4. If the validation stage is story-level and `/src` changed, check whether the story's `QC (Automated)` task requires new Playwright tests. If coverage is missing, author those tests before running the suite.
5. Run only the targeted tests or checks for that story.
6. If the validation stage is epic-end and `/src` changed, run the full regression suites requested by the dispatcher.
7. Summarize pass or fail status for each command or check that ran.
8. If failures occur, capture the failing tests, the likely impacted workstream, and the most relevant error evidence.
9. Hand the results back so the dispatcher can choose the right rework owner and rerun the appropriate QC stage after fixes.
