...
- [ ] **T-1** Add a typed constant `EPIC_MASTERMIND_SUPPORTED_SETS` to `src/app/types.ts` (or a dedicated `src/app/epic-mastermind.ts` constants file) listing the expansion names that contain Epic Mastermind cards.
...
- [ ] **T-2** Add `epicMastermind?: boolean` to the `HistoryRecord` interface in `src/app/types.ts`.

- [ ] **T-3** Update `sanitizeGameRecord` in `src/app/state-store.ts` to normalise the new field.
...
- [ ] **T-4** Test: Add unit tests to `test/epic71-epic-mastermind.test.mjs` covering:
...
- [ ] **T-5** QC (Automated): `npm run lint` passes; `npm test` exits 0.

---

## Story 71.2 — Update the setup generator for Epic Mastermind mode

- [ ] **T-6** Add `epicMastermind?: boolean` to the `GenerateSetupOptions` interface in `src/app/setup-generator.ts`.

- [ ] **T-7** Filter the mastermind pool in `generateSetup` within `src/app/setup-generator.ts`: when `epicMastermind` is `true`, restrict the mastermind pool to cards whose owning set name is in `EPIC_MASTERMIND_SUPPORTED_SETS` **and** that carry the Epic Mastermind designation (use whatever card-level boolean/tag is appropriate in `MastermindRuntime`; add the field if absent).
  - The standard path (no flag or `epicMastermind: false`) must be completely unaffected.

- [ ] **T-8** Add an explicit error path in `generateSetup` in `src/app/setup-generator.ts`: if `epicMastermind` is `true` but the filtered Epic pool is empty, throw a descriptive, localised error (using the locale key `newGame.epicMastermind.noCardsError`) rather than silently falling back to a standard mastermind.

- [ ] **T-9** Test: Add unit tests to `test/epic71-epic-mastermind.test.mjs` covering:
...
- [ ] **T-10** QC (Automated): `npm run lint` passes; `npm test` exits 0.

---

## Story 71.3 — Add the Epic Mastermind opt-in UI control to setup view

- [ ] **T-11** Add a toggle/checkbox control for Epic Mastermind mode inside `src/components/NewGameTab.svelte`.
...
- [ ] **T-12** Persist the toggle value so it survives a page reload.
...
- [ ] **T-13** Test: Add Playwright tests to `test/epic71-epic-mastermind.test.mjs` (or a dedicated `test/epic71-epic-mastermind-e2e.test.mjs`) covering:
...
- [ ] **T-14** QC (Automated): `npm run lint` passes; `npx playwright test` exits 0.

---

## Story 71.4 — Persist and safely read the epicMastermind flag in history records

- [ ] **T-15** Update the "Accept & Log" path in `src/components/NewGameTab.svelte` (and/or the record-creation helper in `src/app/state-store.ts`) to write `epicMastermind: epicMastermindEnabled` (the current toggle value as a boolean) onto every new `HistoryRecord`.

- [ ] **T-16** Confirm that `sanitizeGameRecord` in `src/app/state-store.ts` already handles legacy records without the field (normalising to `false`) per T-3; no additional change needed if T-3 is complete.

- [ ] **T-17** Test: Add integration tests to `test/epic71-epic-mastermind.test.mjs` covering:
...
- [ ] **T-18** QC (Automated): `npm run lint` passes; `npm test` exits 0.

---

## Story 71.5 — Display Epic Mastermind indicator and history grouping mode

- [ ] **T-19** Add a visual indicator (badge, icon, or label) to the history list item template in `src/components/HistoryTab.svelte`.
...
- [ ] **T-20** Add `{ id: 'epic-mastermind', label: 'history.group.epic-mastermind' }` to the `HISTORY_GROUPING_MODES` array in `src/app/history-utils.ts`.

- [ ] **T-21** Implement the `'epic-mastermind'` branch inside the `groupHistoryBy` (or equivalent) function in `src/app/history-utils.ts`:
...
- [ ] **T-22** Verify that `normalizeHistoryGroupingMode('epic-mastermind')` returns `'epic-mastermind'` in `src/app/history-utils.ts` (no change needed if the function already validates against `HISTORY_GROUPING_MODES`; add a targeted assertion in tests).

- [ ] **T-23** Test: Add Playwright tests covering:
...
- [ ] **T-24** QC (Automated): `npm run lint` passes; `npx playwright test` exits 0.

---

## Story 71.6 — Add and translate locale strings for Epic Mastermind UI

- [ ] **T-25** Add all new locale keys to `src/app/locales/en.ts`:
...
- [ ] **T-26** Copy all seven keys into `src/app/locales/fr.ts` with French translations.

- [ ] **T-27** Copy all seven keys into `src/app/locales/de.ts` with German translations.

- [ ] **T-28** Copy all seven keys into `src/app/locales/es.ts` with Spanish translations.

- [ ] **T-29** Copy all seven keys into `src/app/locales/ja.ts` with Japanese translations.

- [ ] **T-30** Copy all seven keys into `src/app/locales/ko.ts` with Korean translations.

- [ ] **T-31** Test: Verify no missing-key lint warnings by confirming each locale file's key set is a superset of `en.ts` (use the existing locale-completeness check if present, or add a targeted assertion to `test/epic71-epic-mastermind.test.mjs`).

- [ ] **T-32** QC (Automated): `npm run lint` passes.
