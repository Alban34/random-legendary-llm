...
- [ ] Open `src/components/NewGameTab.svelte` and locate the `<details>` element that wraps the
...
- [ ] Open `src/app/app-shell.css` and audit the existing Forced Picks CSS rules:
...
- [ ] Verify whether the `<section class="result-card" data-forced-picks-panel>` expands beyond
...
- [ ] Check whether the `.forced-pick-picker-row` flex row (containing a `<select>` and a
...
- [ ] In `src/app/app-shell.css`, apply the minimum CSS changes required to prevent layout overflow
...
- [ ] Confirm the fix holds for both the single-column layout (viewport `< 480px`) and the
  two-column `.forced-picks-pickers-grid` layout (viewport `≥ 480px`) by reviewing the CSS at
  the relevant `@media` breakpoints.

- [ ] Do not touch `src/components/NewGameTab.svelte` HTML structure or JS logic for this story;
  the fix is CSS-only.
...
- [ ] Invoke the UX expert agent (or the project's designer-group prompt at
...
- [ ] Collect the UX expert's written recommendations. The document must cover at minimum:
...
- [ ] Create the recommendation note at
...
- [ ] Update this task list (or this section) with a link to the recommendation document once it
  is created.
...
- [ ] Confirm that `documentation/ux/reports/epic-83-dropdown-ux-recommendations.md` exists and
  that Story 83.2 is marked complete before starting any implementation in this story.

#### CSS implementation tasks

- [ ] In `src/app/app-shell.css`, add or update a `select` (or scoped `.forced-pick-picker-row >
...
- [ ] If the recommendations call for a focus ring change, follow the existing pattern from
...
- [ ] If a caret/chevron indicator is approved, implement it without any background-image data
...
- [ ] Do not introduce any new JavaScript, Svelte reactivity, or changes to
  `src/components/NewGameTab.svelte` for this story; styling changes only.

- [ ] Verify the updated dropdown renders correctly in both dark theme (`:root[data-theme='dark']`)
...
- [ ] Verify the `disabled` state of `<select>` elements (used when `availableOptions.length === 0`
...
- [ ] Ensure the Story 83.1 test block added in `src/app/app-shell.test.ts` covers:
...
- [ ] Ensure the Story 83.3 test block added in `src/app/app-shell.test.ts` covers:
...
- [ ] In `src/components/NewGameTab.test.ts`, add a test block
