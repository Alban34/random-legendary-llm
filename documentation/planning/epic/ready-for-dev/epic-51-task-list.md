...
- [ ] **Task 2.1 — setCatalog entry**: Add the following object to `setCatalog` after the last 2022-year entry and before any 2024 entries:
  ```json
  { "name": "Midnight Sons", "year": 2023, "type": "small-expansion", "aliases": [] }
  ```

- [ ] **Task 2.2 — heroes**: Add 5 entries to `rawCardData.heroes`, each with `"setName": "Midnight Sons"`, `"aliases": []`, `"cardCount": 14`, and `"teams": ["Marvel Knights"]`:
...
- [ ] **Task 2.3 — masterminds**: Add 2 entries to `rawCardData.masterminds` with `"setName": "Midnight Sons"`, `"aliases": []`, `"leadCategory": "villains"`, `"notes": ["Epic Mastermind."]`. Cross-reference BGG to confirm the correct `leadName` for each:
  - `"name": "Lilith, Mother of Demons"` — expected `leadName`: `"Lilin"`
  - `"name": "Zarathos"` — expected `leadName`: `"Fallen"`

- [ ] **Task 2.4 — villainGroups**: Add 2 entries to `rawCardData.villainGroups` with `"setName": "Midnight Sons"`, `"aliases": []`, `"cardCount": 8`:
  - `"name": "Fallen"`
  - `"name": "Lilin"`

- [ ] **Task 2.5 — schemes**: Add 4 entries to `rawCardData.schemes` with `"setName": "Midnight Sons"`, `"aliases": []`, `"constraints": { "minimumPlayerCount": null }`, `"forcedGroups": []`, `"modifiers": []`, `"notes": []`. The dual-phase scheme name must include both phase names separated by ` // `:
...
- [ ] **Task 2.6 — setCatalog entry**: Add the following object to `setCatalog` (2023, after Midnight Sons):
  ```json
  { "name": "The Infinity Saga", "year": 2023, "type": "small-expansion", "aliases": [] }
  ```

- [ ] **Task 2.7 — heroes**: Add 5 entries to `rawCardData.heroes` with `"setName": "The Infinity Saga"`, `"aliases": []`, `"cardCount": 14`. Verify `teams` values against BGG; add `"notes": ["Non-standard card distribution: 1/2/2/3/3/3."]` to each entry to document the deck composition:
...
- [ ] **Task 2.8 — masterminds**: Add 2 entries to `rawCardData.masterminds` with `"setName": "The Infinity Saga"`, `"aliases": []`, `"leadCategory": "villains"`, `"notes": ["Epic Mastermind."]`. Cross-reference BGG to confirm `leadName`:
  - `"name": "Ebony Maw"` — expected `leadName`: `"Children of Thanos"`
  - `"name": "Thanos"` — expected `leadName`: `"Infinity Stones"`

- [ ] **Task 2.9 — villainGroups**: Add 2 entries to `rawCardData.villainGroups` with `"setName": "The Infinity Saga"`, `"aliases": []`, `"cardCount": 8`:
  - `"name": "Children of Thanos"`
  - `"name": "Infinity Stones"`

- [ ] **Task 2.10 — schemes**: Add 4 entries to `rawCardData.schemes` with `"setName": "The Infinity Saga"`, `"aliases": []`, `"constraints": { "minimumPlayerCount": null }`, `"forcedGroups": []`, `"modifiers": []`, `"notes": []`:
...
- [ ] **Task 2.11 — setCatalog entry**: Add the following object to `setCatalog` (2024 group):
  ```json
  { "name": "Weapon X", "year": 2024, "type": "small-expansion", "aliases": [] }
  ```

- [ ] **Task 2.12 — heroes**: Add 4 entries to `rawCardData.heroes` with `"setName": "Weapon X"`, `"aliases": []`, `"cardCount": 14`. Verify `teams` for each against BGG; expected values shown for entries noted in the spec:
...
- [ ] **Task 2.13 — masterminds**: Add 3 entries to `rawCardData.masterminds` with `"setName": "Weapon X"`, `"aliases": []`, `"leadCategory": "villains"`, `"notes": ["Epic Mastermind.", "Expansion includes 10 Enraging Wounds (special wound variant)."]`. Cross-reference BGG to confirm each `leadName`:
  - `"name": "Omega Red"` — verify `leadName`
  - `"name": "Romulus"` — verify `leadName`
  - `"name": "Sabretooth"` — verify `leadName`

- [ ] **Task 2.14 — villainGroups**: Add 2 entries to `rawCardData.villainGroups` with `"setName": "Weapon X"`, `"aliases": []`, `"cardCount": 8`:
  - `"name": "Berserkers"`
  - `"name": "Weapon Plus"`

- [ ] **Task 2.15 — schemes**: Add 3 entries to `rawCardData.schemes` with `"setName": "Weapon X"`, `"aliases": []`, `"constraints": { "minimumPlayerCount": null }`, `"forcedGroups": []`, `"modifiers": []`, `"notes": []`:
...
- [ ] **Task 2.16 — setCatalog entry**: Add the following object to `setCatalog` (2024 group):
  ```json
  { "name": "2099", "year": 2024, "type": "small-expansion", "aliases": [] }
  ```

- [ ] **Task 2.17 — heroes**: Add 5 entries to `rawCardData.heroes` with `"setName": "2099"`, `"aliases": []`, `"cardCount": 14`. Verify `teams` for each against BGG; expected team for spec-noted entries:
...
- [ ] **Task 2.18 — masterminds**: Add 2 entries to `rawCardData.masterminds` with `"setName": "2099"`, `"aliases": []`, `"leadCategory": "villains"`. These are Epic Adapting Masterminds with non-standard card counts; capture this in `notes` and confirm `leadName` values against BGG:
...
- [ ] **Task 2.19 — villainGroups**: Add 2 entries to `rawCardData.villainGroups` with `"setName": "2099"`, `"aliases": []`, `"cardCount": 8`:
  - `"name": "Alchemax Enforcers"`
  - `"name": "False Aesir of Alchemax"`

- [ ] **Task 2.20 — schemes**: Add 4 entries to `rawCardData.schemes` with `"setName": "2099"`, `"aliases": []`, `"constraints": { "minimumPlayerCount": null }`, `"forcedGroups": []`, `"modifiers": []`, `"notes": []`:
...
- [ ] **Task 2.21 — setCatalog entry**: Add the following object to `setCatalog` (2024 group). Verify `type` against BGG — the content volume (8 heroes, 3 masterminds, 4 villain groups, 3 henchman groups) may qualify this as `"large-expansion"`:
  ```json
...
- [ ] **Task 2.22 — heroes**: Add 8 entries to `rawCardData.heroes` with `"setName": "Ant-Man and the Wasp"`, `"aliases": []`, `"cardCount": 14`. Verify `teams` for each against BGG. Note: `"Ant-Man"` is a duplicate name; `setName` distinguishes it from the entry with `setName: "Ant-Man"`:
...
- [ ] **Task 2.23 — masterminds**: Add 3 Transforming Mastermind entries to `rawCardData.masterminds` with `"setName": "Ant-Man and the Wasp"`, `"aliases": []`, `"leadCategory": "villains"`, `"notes": ["Transforming Mastermind."]`. Use the `/` separator in `name` for the two-form name (matching the existing convention used by e.g. `"General \"Thunderbolt\" Ross / Red Hulk"`). Cross-reference BGG to confirm each `leadName`:
...
- [ ] **Task 2.24 — villainGroups**: Add 4 entries to `rawCardData.villainGroups` with `"setName": "Ant-Man and the Wasp"`, `"aliases": []`, `"cardCount": 8`:
  - `"name": "Armada of Kang"`
  - `"name": "Cross Technologies"`
  - `"name": "Ghost Chasers"`
  - `"name": "Quantum Realm"`

- [ ] **Task 2.25 — henchmanGroups**: Add 3 entries to `rawCardData.henchmanGroups` with `"setName": "Ant-Man and the Wasp"`, `"aliases": []`, `"cardCount": 10`:
  - `"name": "Quantum Hound"`
  - `"name": "Quantumnauts"`
  - `"name": "Tardigrade"`

- [ ] **Task 2.26 — schemes**: Add 4 entries to `rawCardData.schemes` with `"setName": "Ant-Man and the Wasp"`, `"aliases": []`, `"constraints": { "minimumPlayerCount": null }`, `"forcedGroups": []`, `"modifiers": []`. Add bystander notes where relevant (Special Bystanders: Agent Jimmy Woo ×2, Maggie Lang ×2, Officer Jim Paxton ×1, Young Cassie Lang ×2; verify per-scheme placement against BGG):
...
- [ ] **Test (Story 2)**: Run `npm run lint` and `npm test` and confirm all pass.
- [ ] **QC (Automated — Story 2)**: Verify the following against `canonical-game-data.json`:
...
- [ ] **Task 3.1 — date audit**: For every entry in `setCatalog`, record its current `year` value and cross-reference it against the BGG listing for that expansion (search by exact `name`; use the "Year Published" field on BGG, or the official Legendary wiki as a secondary source). Produce a local list of every entry where the recorded `year` does not match the verified first-release year.

- [ ] **Task 3.2 — apply corrections**: For each entry identified in Task 3.1, update its `year` field in `setCatalog` to the verified release year. Change only the `year` field; leave `name`, `type`, and `aliases` untouched.

- [ ] **Test (Story 3)**: Run `npm run lint` and `npm test` and confirm all pass.
- [ ] **QC (Automated — Story 3)**: Verify the following against the pre- and post-edit snapshots of `canonical-game-data.json`:
