...
- [ ] Open `src/app/collection-utils.ts` and verify that `CARD_CATEGORIES` lists exactly five
  entries in the canonical order:
  `heroes` → `masterminds` → `villainGroups` → `henchmanGroups` → `schemes`.
- [ ] Confirm that each `CARD_CATEGORIES` entry carries a `labelKey` pointing to an existing
  `common.*` locale key (e.g. `common.heroes`, `common.masterminds`, `common.villainGroups`,
  `common.henchmanGroups`, `common.schemes`).
- [ ] Confirm that `getCardsByCategory(pools)` is a pure function with no side effects — it only
  reads from `pools` and returns a new array.
- [ ] Add a single-line code comment directly above the `CARD_CATEGORIES` constant in
...
- [ ] **Test:** Verify that `src/app/collection-utils.test.ts` already contains the test
...
- [ ] Open `src/components/CardBrowserByCategory.svelte` and read the current template. Each
  category group is rendered as `<section data-category={category.categoryId}>` with a bare
  `<h3>` heading.
- [ ] Replace each `<section>` / `<h3>` group with the `<details>` / `<summary>` pattern used in
...
- [ ] Do **not** introduce any hardcoded colour values, font sizes, or spacing values.
  Use only existing design-token variables (e.g. `--space-*`) or the shared `.history-group` class
  from `src/app/app-shell.css`.
- [ ] Confirm that the `locale` prop is already threaded into `CardBrowserByCategory.svelte`
  (it is declared as `let { pools, locale }: { pools: GamePool; locale: LocaleTools } = $props();`).
  No prop changes are required.
- [ ] Verify the empty-state path (`pools.sets.length === 0`) is unaffected by the template change.
- [ ] **Test:** After the change, manually inspect the rendered output in a dev server (or via the
...
- [ ] In `src/app/locales/en.ts`, confirm that `common.heroes`, `common.masterminds`,
  `common.villainGroups`, `common.henchmanGroups`, and `common.schemes` are all present.
- [ ] In `src/app/locales/fr.ts`, confirm the same five `common.*` keys are present.
- [ ] In `src/app/locales/de.ts`, confirm the same five `common.*` keys are present.
- [ ] In `src/app/locales/ja.ts`, confirm the same five `common.*` keys are present.
- [ ] In `src/app/locales/ko.ts`, confirm the same five `common.*` keys are present.
- [ ] In `src/app/locales/es.ts`, confirm the same five `common.*` keys are present.
- [ ] If any key is absent from any locale file, add the missing entry immediately after the
  nearest sibling `common.*` key block in that file, using a translation consistent with the
  existing vocabulary in that locale.
- [ ] **Test:** Run `npm run lint` and confirm it passes with no errors. This is the mandatory
...
- [ ] `'CARD_CATEGORIES has exactly 5 entries in canonical order'` — canonical order and
  `labelKey` presence.
- [ ] `'getCardsByCategory returns exactly 5 category buckets'` — bucket count for a
  single owned set.
- [ ] `'getCardsByCategory empty set list returns 5 empty-cards buckets'` — empty-collection
...
- [ ] Create (or extend) the test file and add a test:
...
- [ ] Run `npm run lint` locally to confirm no lint regressions before handing off to Story 84.5.
- [ ] **Test (gate):** All new and existing tests in `src/app/collection-utils.test.ts` and the
