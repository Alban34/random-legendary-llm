...
- [ ] **QC (Automated)** — Run `npm run lint` then `npm test -- --testPathPattern="epic73|epic3-setup-generator|epic53-solo-scheme-eligibility"` to verify generator suppression and confirm no regressions.
...
- [ ] **Suppress `leadEntity` in solo mode** (`src/app/setup-generator.ts`, line ~894) — change the `leadEntity` assignment in `tryMastermindForScheme` to return `null` when `isSoloMode(template)` is true (reuse the helper from Story 1):
  ```
  // Before:
...
- [ ] **No change needed to `src/components/NewGameTab.svelte`** — The result view at line ~457 already branches on `currentSetup.mastermind.leadEntity`. Once `leadEntity` is null in solo mode, the view will automatically render `locale.t('common.noMandatoryLead')` and the `★ Mandatory Lead` pill (line ~459) will be hidden. No further Svelte edits are required.

- [ ] **Test — Add Story 2 cases to `test/epic73-solo-always-leads.test.mjs`:**
...
- [ ] **QC (Automated)** — Run `npm run lint` then `npm test -- --testPathPattern="epic73"` to validate leadEntity suppression.
...
- [ ] **`src/app/solo-rules.ts` line ~16** — Remove `'newGame.soloRules.standard.alwaysLeads'` from the `'standard'` case array. Array shrinks from 4 → 3 items.

- [ ] **`src/app/solo-rules.ts` line ~24** — Remove `'newGame.soloRules.advancedSolo.alwaysLeads'` from the `'advanced-solo'` case array. Array shrinks from 5 → 4 items.

- [ ] **`src/app/solo-rules.ts` line ~33** — Remove `'newGame.soloRules.standardV2.alwaysLeads'` from the `'standard-solo-v2'` case array. Array shrinks from 6 → 5 items.

### Locale files — remove the three keys from all 12 files

- [ ] **`src/app/locales/en.ts`** — Remove lines for `newGame.soloRules.standard.alwaysLeads` (~line 454), `newGame.soloRules.advancedSolo.alwaysLeads` (~line 459), and `newGame.soloRules.standardV2.alwaysLeads` (~line 465).

- [ ] **`src/app/locales/en.mjs`** — Same three keys, same line numbers.

- [ ] **`src/app/locales/fr.ts`** — Remove the same three keys.

- [ ] **`src/app/locales/fr.mjs`** — Remove the same three keys.

- [ ] **`src/app/locales/de.ts`** — Remove the same three keys.

- [ ] **`src/app/locales/de.mjs`** — Remove the same three keys.

- [ ] **`src/app/locales/es.ts`** — Remove the same three keys.

- [ ] **`src/app/locales/es.mjs`** — Remove the same three keys.

- [ ] **`src/app/locales/ja.ts`** — Remove the same three keys.

- [ ] **`src/app/locales/ja.mjs`** — Remove the same three keys.

- [ ] **`src/app/locales/ko.ts`** — Remove the same three keys.

- [ ] **`src/app/locales/ko.mjs`** — Remove the same three keys.

### Test updates — `test/epic57-solo-rules-panel.test.mjs`

This file has six assertions that will fail after the keys are removed:

- [ ] **Line 66** — Remove `assert.equal(items[3], 'newGame.soloRules.standard.alwaysLeads')`. The `standard` array now ends at index 2 (`eachOtherPlayer`).

- [ ] **Line 76** — Remove `assert.equal(items[4], 'newGame.soloRules.advancedSolo.alwaysLeads')`. The `advanced-solo` array now ends at index 3 (`eachOtherPlayer`).

- [ ] **Line 87** — Remove `assert.equal(items[5], 'newGame.soloRules.standardV2.alwaysLeads')`. The `standard-solo-v2` array now ends at index 4 (`mastermindAbility`).

- [ ] **Lines ~110–113** (`computeSoloRulesItems` length assertions) — Update:
...
- [ ] **Lines ~168–210** (`EN_MESSAGES contains all 17 solo rules locale keys`) — Remove the three `alwaysLeads` keys from the `expectedKeys` array and update the test description from `'17 solo rules locale keys'` to `'14 solo rules locale keys'`.

- [ ] **Test — Add Story 3 assertion to `test/epic73-solo-always-leads.test.mjs`:**
...
- [ ] **QC (Automated)** — Run `npm run lint` then `npm test -- --testPathPattern="epic73|epic57-solo-rules-panel"` to confirm key removal and no missing-key warnings.
