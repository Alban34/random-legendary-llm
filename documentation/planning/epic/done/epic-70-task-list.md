...
- [x] **T-1** Add `preferredExpansionId: string | null` to the `ForcedPicks` interface in `src/app/forced-picks-utils.ts`.
  - Insert the field after `henchmanGroupIds` to keep the existing field order stable.

- [x] **T-2** Update `createEmptyForcedPicks` in `src/app/forced-picks-utils.ts` to include `preferredExpansionId: null` in the returned object.

- [x] **T-3** Update `normalizeForcedPicks` in `src/app/forced-picks-utils.ts` to normalise the new field:
...
- [x] **T-4** Update `hasForcedPicks` in `src/app/forced-picks-utils.ts` to return `true` when `normalized.preferredExpansionId` is non-null.

- [x] **T-5** Test: Add or extend unit tests in `test/epic70-preferred-expansion.test.mjs` (created in full under Story 70.6) to cover Story 70.1 obligations:
...
- [x] **T-6** QC (Automated): Run `npm run lint` (must pass with zero errors), then run `npm test` (must exit 0).

---

## Story 70.2 — Update the setup generator for preferred expansion fill

- [x] **T-7** Read `src/app/setup-generator.ts` lines covering the per-slot draw logic to locate where the lowest available play-count tier is selected and cards are drawn for each slot type (heroes, villain groups, henchman groups, scheme, mastermind).

- [x] **T-8** Update `src/app/setup-generator.ts` to apply the preferred-expansion tiebreaker within each play-count tier for every slot not already claimed by an individually forced card:
...
- [x] **T-9** Test: Add unit tests in `test/epic70-preferred-expansion.test.mjs` to cover Story 70.2 obligations:
...
- [x] **T-10** QC (Automated): Run `npm run lint` (must pass with zero errors), then run `npm test` (must exit 0).

---

## Story 70.3 — Verify individually forced cards take absolute priority

- [x] **T-11** Add unit tests in `test/epic70-preferred-expansion.test.mjs` to cover Story 70.3 obligations:
...
- [x] **T-12** QC (Automated): Run `npm run lint` (must pass with zero errors), then run `npm test` (must exit 0).

---

## Story 70.4 — Add UI controls for preferred expansion in the forced-picks panel

- [x] **T-13** Locate the "Forced picks" panel markup in `src/components/App.svelte` (search for `newGame.forcedPicks.title`). Add a preferred-expansion selector sub-section below the existing per-card-type pickers:
...
- [x] **T-14** Display the currently active preferred expansion inline when `preferredExpansionId` is non-null in `src/components/App.svelte`:
...
- [x] **T-15** Wire the set and clear interactions to the app state in `src/components/App.svelte`:
...
- [x] **T-16** Test: Write Playwright tests covering Story 70.4 obligations:
...
- [x] **T-17** QC (Automated): Run `npm run lint` (must pass with zero errors), then run `npx playwright test` (must exit 0).

---

## Story 70.5 — Add and translate locale strings for preferred expansion UI

- [x] **T-18** Add all new locale keys to `src/app/locales/en.mjs` under the `newGame.forcedPicks.*` namespace, following the existing key naming conventions:
...
- [x] **T-19** Translate all five new keys added in T-18 into each of the five remaining locale files:
...
- [x] **T-20** Test: Verify no missing-key lint warnings by confirming each of the five new keys appears in all six locale files. A quick `grep` across `src/app/locales/*.mjs` for each key name is sufficient evidence.

- [x] **T-21** QC (Automated): Run `npm run lint` (must pass with zero errors, including any locale-completeness lint rules).

---

## Story 70.6 — Automated regression tests for preferred expansion end-to-end

- [x] **T-22** Create `test/epic70-preferred-expansion.test.mjs`. The file must contain test cases for all six regression scenarios listed in the epic spec:
...
- [x] **T-23** QC (Automated): Run `npm run lint` (must pass with zero errors), then run `npm test` (must exit 0 with all cases in `test/epic70-preferred-expansion.test.mjs` green).
