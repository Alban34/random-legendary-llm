...
- [x] Run `npm test` before touching any file and record the total number of passing tests as the
...
- [x] Confirm that each import in the table below still resolves to the correct path by grepping for
  the function names listed in the "Primary functions tested" column.

- [x] Confirm that no test file in `test/` (excluding `test/playwright/`) is missing from the table.
...
| `test/epic36-version-storage-disclosure.test.ts` | `src/components/App.svelte` + vite.config.ts (version injection) → merge into `src/components/App.test.ts` | `src/components/BackupTab.svelte` (storage-version disclosure, schema-version display) → `src/components/BackupTab.test.ts` | — | — |
...
- [x] **Test (manual):** Verify the target inventory list above matches what was actually created
  before closing this story.

- [x] **QC (Automated):** Run `npm run lint` and confirm zero lint errors. No test execution yet.
...
- [x] Move `test/epic1.test.ts` → `src/app/game-data-pipeline.test.ts`; update imports from
  `'../src/app/...'` to `'./'` or `'../'`; update `rootDir` / `__dirname` derived seed paths.
- [x] Move `test/epic2-state.test.ts` → `src/app/state-store.test.ts`; fix relative paths.
- [x] Move `test/epic4-shell-navigation.test.ts` → `src/app/app-tabs.test.ts`; fix relative paths.
- [x] Move `test/epic5-browse-extensions.test.ts` → `src/app/browse-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic6-collection-management.test.ts` → `src/app/collection-utils.test.ts`; fix
  relative paths.
- [x] Move `test/epic7-new-game-experience.test.ts` → `src/app/new-game-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic8-history-usage-reset.test.ts` → `src/app/history-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic12-score-history.test.ts` → `src/app/result-utils.test.ts`; fix relative paths.
- [x] Move `test/epic13-backup-portability.test.ts` → `src/app/backup-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic14-stats-dashboard.test.ts` → `src/app/stats-utils.test.ts`; fix relative paths.
- [x] Move `test/epic18-theme-personalization.test.ts` → `src/app/theme-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic19-localization.test.ts` → `src/app/localization-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic24-toast-behavior.test.ts` → `src/app/preferences-actions.test.ts`; fix
  relative paths.
- [x] Move `test/epic42-bgg-import.test.ts` → `src/app/bgg-import-utils.test.ts`; fix relative paths.
- [x] Move `test/epic45-myludo-import.test.ts` → `src/app/myludo-import-utils.test.ts`; fix relative
  paths.
- [x] Move `test/epic56-standard-v2-solo.test.ts` → `src/app/setup-rules.test.ts`; fix relative
  paths.
- [x] Move `test/epic57-solo-rules-panel.test.ts` → `src/app/solo-rules.test.ts`; fix relative paths.
- [x] Move `test/epic70-preferred-expansion.test.ts` → `src/app/forced-picks-utils.test.ts`; fix
  relative paths.

### Tasks — `src/app/locales/` renames

- [x] Move `test/epic75-locale-sync-a11y.test.ts` → `src/app/locales/locales.test.ts`; fix relative
  paths (source imports are already absolute module imports, no path changes needed for those).

### Tasks — `src/components/` renames

- [x] Move `test/epic23-stats-simplification.test.ts` → `src/components/HistoryTab.test.ts`; fix
...
- [x] Move `test/epic72-active-expansions-collapsed.test.ts` → `src/components/NewGameTab.test.ts`;
  fix relative paths similarly.

### Tasks — confirm deletions

- [x] Delete all 20 original `test/epic*.test.ts` files handled in this story (the moved files).
...
- [x] **Test (manual):** After this story, run `npm test` (update the config first per Story 82.4 if
  needed) and confirm at minimum the 20 moved files are discovered and their tests still pass.

- [x] **QC (Automated):** Run `npm run lint`. Confirm zero errors on moved files.
...
- [x] Create `src/app/setup-generator.test.ts` using the content of `test/epic3-setup-generator.test.ts`
  as the seed file (primary setup-generator functions); fix relative paths.
- [x] Create `src/app/app-renderer.test.ts`; seed with app-renderer structural assertions extracted
  from `test/epic9-notifications-accessibility.test.ts` and `test/epic27-shell-debug-polish.test.ts`.
- [x] Create `src/app/new-game-vm.test.ts`; seed with `new-game-vm.svelte.ts` toast-behavior
  assertions extracted from `test/epic16-notification-refinements.test.ts`.
- [x] Create `src/app/app-shell.test.ts`; seed with CSS token assertions extracted from
  `test/design-system-epic1-foundation.test.ts`.
- [x] Create `src/app/focus-utils.test.ts`; seed with `focus-utils.ts` assertions extracted from
  `test/design-system-rollout.test.ts`.
- [x] Create `src/components/App.test.ts`; seed with `App.svelte` assertions extracted from
...
- [x] Create `src/components/BackupTab.test.ts`; seed with `BackupTab.svelte` portability-panel
  assertions extracted from `test/epic-ux6-backup-safety.test.ts`.
- [x] Create `src/components/BrowseTab.test.ts`; seed with `BrowseTab.svelte` structural assertions
  extracted from `test/epic21-browse-polish.test.ts`.
- [x] Create `src/components/CollectionTab.test.ts`; seed with `CollectionTab.svelte` assertions
  extracted from `test/epic78-ui-layout-polish.test.ts`.
- [x] Create `src/data/canonical-game-data.test.ts`; seed with game-data type-classification
...
- [x] Merge `test/epic15-forced-picks.test.ts` → `src/app/setup-generator.test.ts`
  (forced-picks integration in `generateSetup`/`validateSetupLegality`).
- [x] Merge `test/epic53-solo-scheme-eligibility.test.ts` → `src/app/setup-generator.test.ts`
  (`validateSetupLegality` solo scheme eligibility).
- [x] Merge `test/epic11-play-modes.test.ts` new-game-utils portion → `src/app/new-game-utils.test.ts`;
  merge setup-rules portion (`resolveSetupTemplate`) → `src/app/setup-rules.test.ts`.
- [x] Merge `test/epic20-history-grouping.test.ts` → `src/app/history-utils.test.ts`.
- [x] Merge `test/epic34-history-grouping.test.ts` → `src/app/history-utils.test.ts`
  (HISTORY_GROUPING_MODES, normalizeHistoryGroupingMode).
- [x] Merge `test/epic43-expansion-attribution.test.ts` → `src/app/history-utils.test.ts`
  (`formatHistorySummary` expansion-attribution assertions).
- [x] Merge `test/epic47-history-outcome-filter.test.ts` → `src/app/history-utils.test.ts`
  (`filterHistoryByOutcome`).
- [x] Merge `test/epic44-card-browser.test.ts` → `src/app/collection-utils.test.ts`
  (CARD_CATEGORIES, getCardsByCategory, getCardsByExpansion).
- [x] Merge `test/epic46-active-filter.test.ts` → `src/app/state-store.test.ts`
  (setActiveSetIds, clearActiveSetIds, sanitizePersistedState SCHEMA_VERSION).
- [x] Merge `test/epic60-sets-browser-sort.test.ts` → `src/app/browse-utils.test.ts`
  (BROWSE_SORT_OPTIONS, sort-key variants of filterBrowseSets).
- [x] Merge `test/epic37-small-improvements.test.ts` → `src/app/locales/locales.test.ts`
  (per-locale tab-string coverage, package.json version assertion).
- [x] Merge `test/epic3-setup-generator.test.ts` setup-rules portion (`resolveSetupTemplate` calls) →
  `src/app/setup-rules.test.ts`; delete the original epic3 file after the seed file (above) was
  created from it.
- [x] Merge `test/epic9-notifications-accessibility.test.ts` state-store/setup-generator runtime
...
- [x] Merge `test/epic16-notification-refinements.test.ts` app-renderer structural assertions →
  `src/app/app-renderer.test.ts`; delete original file.
- [x] Merge `test/epic17-onboarding-information-architecture.test.ts`: state-store parts →
  `src/app/state-store.test.ts`; app-renderer parts → `src/app/app-renderer.test.ts`; delete
  original file.
- [x] Merge `test/epic21-browse-polish.test.ts` app-renderer structural assertions →
  `src/app/app-renderer.test.ts`; delete original file.
- [x] Merge `test/epic22-catalog-ordering.test.ts` browse-utils sort assertion →
  `src/app/browse-utils.test.ts`; delete original file.
- [x] Merge `test/epic25-header-new-game.test.ts`: app-shell.css assertions →
  `src/app/app-shell.test.ts`; NewGameTab button assertion → `src/components/NewGameTab.test.ts`;
  delete original file.
- [x] Merge `test/epic26-classification-corrections.test.ts` browse-utils/collection-utils structural
  checks → respective module test files; delete original file.
- [x] Merge `test/epic27-shell-debug-polish.test.ts` setup-generator structural assertion →
  `src/app/setup-generator.test.ts`; delete original file.
- [x] Merge `test/epic36-version-storage-disclosure.test.ts`: App.svelte/vite.config.ts version parts
  → `src/components/App.test.ts`; BackupTab disclosure → `src/components/BackupTab.test.ts`; delete
  original file.
- [x] Merge `test/epic71-epic-mastermind.test.ts`: setup-generator mastermind lead tests →
...
- [x] Merge `test/epic73-solo-always-leads.test.ts`: setup-generator solo always-leads →
  `src/app/setup-generator.test.ts`; solo-rules assertions → `src/app/solo-rules.test.ts`; delete
  original file.
- [x] Merge `test/epic74-forced-hero-team.test.ts`: forced-picks-utils forcedTeam →
  `src/app/forced-picks-utils.test.ts`; setup-generator buildOwnedPools/generateSetup forced-team →
  `src/app/setup-generator.test.ts`; delete original file.
- [x] Merge `test/epic78-ui-layout-polish.test.ts`: HistoryTab cases → `src/components/HistoryTab.test.ts`;
  ...
  delete original file.
- [x] Merge `test/design-system-epic1-foundation.test.ts` theme-utils assertions →
  `src/app/theme-utils.test.ts`; delete original file.
- [x] Merge `test/design-system-rollout.test.ts`: app-shell.css typography →
...
- [x] Merge `test/epic-ux6-backup-safety.test.ts` app-renderer/app-shell structural assertions →
  `src/app/app-renderer.test.ts`; delete original file.

### Confirm clean-up

- [x] Verify the `test/` directory contains only `test/playwright/` (no `*.test.ts` files remain at
  the top level of `test/`).
- [x] Verify the 31 module-keyed test files listed in the Story 82.1 inventory all exist.

- [x] **Test (manual):** With the vitest config updated (Story 82.4), confirm `npm test` finds all 31
  files and no test is reported as missing or skipped.

- [x] **QC (Automated):** Run `npm run lint`. Confirm zero errors on all 31 new test files.
...
- [x] Open `vitest.config.ts`. The current `include` glob was:
  ```js
...
- [x] Verify the `coverage.include` already covers `src/**/*.ts` and `src/**/*.js`; no change
  required to the coverage block.

- [x] Check `package.json` for any `test:*` scripts that reference hard-coded paths inside `test/`
...
- [x] Verify there are no `tsconfig.json` `include`/`exclude` patterns that explicitly reference
  `test/**` in a way that would need updating now that test files live in `src/`.

- [x] **Test (manual):** Run `npm test -- --reporter=verbose` and confirm every one of the 31 target
  test files appears in the output.

- [x] **QC (Automated):** Run `npm run lint` after the config change. Confirm no new lint errors
...
- [x] Run `npm test` and compare the total passing test count to the pre-refactor baseline recorded
  in the pre-flight step. The counts must be equal.

- [x] Confirm that no test file is collected zero times (i.e. no accidental empty file or
  wrong-glob exclusion).

- [x] Confirm that coverage output (if generated) still reports `src/**` files and has not regressed
  from the pre-refactor run.

- [x] Confirm that no import in any test file still references a path that begins with `'../test/'`.

- [x] Confirm that no file named `epic*.test.ts` or `design-system-*.test.ts` or
  `epic-ux*.test.ts` exists anywhere under `src/` or `test/` (excluding `test/playwright/`).

- [x] **Test (manual):** Run the full suite locally and record the final passing count.

- [x] **QC (Automated):** Run `npm run lint` then `npm test`. Both must exit 0. Pass the final test
