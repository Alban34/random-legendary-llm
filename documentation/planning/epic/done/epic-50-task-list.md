...
- [x] Open `index.html` at the repo root and record the exact `href` value of the `<link rel="manifest">` tag on line 74 — confirm it reads `/manifest.webmanifest` (absolute root-relative path, not base-path-aware)
- [x] Run `npm run build` and inspect `dist/index.html` — confirm whether Vite rewrites the manifest `href` to include `/random-legendary-llm/` or leaves it as `/manifest.webmanifest`
- [x] Run `npm run build` and confirm `dist/manifest.webmanifest` exists (copied from `public/manifest.webmanifest` by Vite)
- [x] Run `npm run build` and confirm `dist/sw.js` exists and that the `%%SW_CACHE_VERSION%%` and `/*%%SW_PRECACHE_URLS%%*/[]` placeholders have been replaced by the `swInjectPlugin` in `vite.config.js`
- [x] Confirm that `dist/sw.js` precache URL list includes `/random-legendary-llm/manifest.webmanifest` — if the manifest is not in the precache list, offline loads will not serve it
- [x] Verify `src/app/browser-entry.mjs` registers the service worker via `import.meta.env.BASE_URL + 'sw.js'` and confirm that at build time `BASE_URL` resolves to `/random-legendary-llm/`, making the registration path `/random-legendary-llm/sw.js`
...
- [x] In `index.html` line 74, change `href="/manifest.webmanifest"` to `href="%BASE_URL%manifest.webmanifest"` so Vite substitutes the correct base path (`/random-legendary-llm/`) at build time and `/` during dev
- [x] Run `npm run build` and verify `dist/index.html` now contains `href="/random-legendary-llm/manifest.webmanifest"` (not the bare `/manifest.webmanifest`)
- [x] Confirm `dist/sw.js` precache URL array includes `/random-legendary-llm/manifest.webmanifest` after the build (the `collectFiles` walk in `swInjectPlugin` covers `dist/manifest.webmanifest`, which maps to `/random-legendary-llm/manifest.webmanifest`)
