...
- Story 3: `eslint.config.mjs` contains a separate config block matching `['src/**/*.ts']` that uses `typescript-eslint` parser and recommended rules; `.svelte` files are excluded from this block because `tseslint.configs.recommended` includes `prefer-const: error` which clashes with Svelte's `let { ... } = $props()` destructuring pattern; the existing `.mjs` block is unchanged; `npm run lint` exits 0.
