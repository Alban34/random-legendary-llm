...
This file is the **machine-facing game-data specification** derived from the authoritative BoardGameGeek reference pages listed in `documentation/data/sources.md`.
...
> The authoritative external source set for this document is defined in `documentation/data/sources.md`.
...
- `leadCandidates`: optional pre-resolved candidate pool for categorical leads, as `Array<{ category, id }>`
- `notes`: human-readable setup notes retained in the project format when useful

When a canonical entry carries `leadNameFilter` (a non-empty string array), the normalizer populates `leadCandidates` with every globally-known villain group whose name contains any of the filter strings (case-insensitive). `lead` is `null` for these masterminds. At generation time the generator picks one candidate at random from those present in the owned collection; if none are owned, the mastermind is skipped for that attempt. Example: Sinister Six 2099 uses `leadNameFilter: ["Alchemax", "Sinister"]`, covering any Alchemax or Sinister-named villain group.
...
- `Secret Invasion of the Skrull Shapeshifters` forces `Skrulls` and uses 6 heroes (not the standard count)
- `Midtown Bank Robbery` changes the bystander count

> **Epic 104 expansion:** Structured setup rules were added for 13 additional schemes across Core Set, Marvel Studios Phase 1, Marvel Studios' What If...?, and Villains. Examples include: `Asgard Under Siege` (extra henchman group), `Breach the Nexus of All Realities` (conditional extra villain group for 1–2 players), `Trash Earth with Hugest Party Ever` (forced villain group and required hero), `Mass Produce War Machine Armor` (forced henchman group), `Replace Earth's Leaders with Killbots` / `Replace Earth's Leaders with HYDRA` (18 bystanders), and side-stack bystander notes for `Infiltrate the Lair with Spies` and `Graduation at Xavier's X-Academy`. See `documentation/data/data-model.md` for the full `RuleModifier` vocabulary.

> **Epic 105 expansion:** Machine-readable modifiers and `forcedGroups` entries were added or corrected for 28+ schemes across Secret Wars Volume 1 & 2, Civil War, Dark City, X-Men, Midnight Sons, Villains, and other sets. A new `conditional-add-hero` modifier type was introduced (used by Age of Ultron: add 1 hero for 4–5 players). `Secret Invasion of the Skrull Shapeshifters` was corrected to `set-min-heroes: 6` (reverted from an incorrect Epic 104 value of 5), with **no** additional `add-hero` modifier — the scheme uses a flat minimum of 6 heroes. `Enslave Minds with the Chitauri Scepter` gained `set-min-heroes: 6` and `add-hero: 1`.

> **Correction (clean-code cleanup, 3 June 2026):** A drifted `add-hero: 1` modifier on `Secret Invasion of the Skrull Shapeshifters` (introduced after this epic and briefly raising its hero count to 7) was removed to restore alignment with the seed source-of-truth (`tools/build_epic1_seed.py` = `set-min-heroes: 6`), the v2.1.1 baseline, and the E2E contract. The shipped scheme uses `set-min-heroes: 6` only, with no `add-hero` modifier. A `require-hero-name-match-count` modifier type was introduced for schemes requiring a hero matched by name pattern (e.g. `Condition Logan into Weapon X`, `Shoot Hulk into Space`). See `documentation/data/data-model.md` for the full `RuleModifier` vocabulary.

> **Epic 106 expansion:** Human-readable `notes` entries were added or restored for 30 schemes. Two notes originally added by Epic 104 and later rolled back — `Infiltrate the Lair with Spies` and `Graduation at Xavier's X-Academy` — were restored. Thirteen additional schemes that had empty notes received new entries across Revelations, Messiah Complex, Deadpool, Fear Itself, Guardians of the Galaxy, Midnight Sons, and The Infinity Saga. Fifteen schemes with OCR-corrupted or truncated source text received `TODO`-marked notes pending card-text verification. No `modifiers` or `forcedGroups` were changed in this epic.
