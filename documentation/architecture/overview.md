...
- `src/app/types.ts` — barrel re-export file (Epic 62, refactored); re-exports all domain type declarations from the split type files below for backwards compatibility; migrated modules in Epics 63–67 continue to import from this file without changes
- `src/app/types-game-data.ts` — game data shape declarations: canonical source types (`GameSet`, `HeroCard`, etc.) and runtime index types (`HeroRuntime`, `RuntimeIndexes`, etc.) plus `EPIC_MASTERMIND_SUPPORTED_SETS`
- `src/app/types-app-state.ts` — application state declarations: `AppState`, `PlayMode`, `ThemeId`, `LocaleId`, `Preferences`, `HistoryRecord`, `GameResult`, etc.
- `src/app/types-setup.ts` — setup and import return type declarations: `GeneratedSetup`, `SetupRequirements`, `BggMatchResult`, `MyludoMatchResult`
- `src/app/types-storage.ts` — storage and backup type declarations: `StorageAdapter`, `StorageOperationResult`, `BackupPayload`, `BackupSummary`
- `src/app/types-locale.ts` — locale type declarations: `LocaleTools`, `LocaleId`
- `src/app/types-ui.ts` — UI and app integration type declarations: theme types, `AppTab`, `ModalConfig`, `OnboardingActions`, `AppPersistenceState`, `StagedBackup`
...
- `src/app/game-data-pipeline.ts` — pipeline orchestration: `createEpic1Bundle(seed)`, `runEpic1Tests`, `slugify`, `clone`, type exports (`Epic1Bundle`, `PipelineRuntime`); data building and normalization extracted to `game-data-normalizer.ts`; index building extracted to `game-data-indexes.ts`
- `src/app/game-data-normalizer.ts` — canonical data building and runtime normalization: `buildCanonicalSourceData`, `normalizeGameData`, all card-builder helpers and reference-resolution functions
- `src/app/game-data-indexes.ts` — index building and validation: `buildIndexes`, `validateNormalizedData`
- `src/app/state-store.ts` — state mutation functions and the versioned root state key (`legendary_state_v1`); exports `setActiveSetIds(state, ids)`, `clearActiveSetIds(state)`, `deactivateAllSets(state)` (added in Epic 46), `toggleOwnedSet`, `acceptGameSetup`, `updateGameResult`, and all other state mutators; default-state factories, sanitization, IO, and storage adapter extracted into dedicated files below
- `src/app/state-defaults.ts` — default-state factory functions: `createDefaultState`, `createDefaultUsageState`, `createDefaultPreferences`
- `src/app/state-sanitizer.ts` — state sanitization functions: `sanitizeStateCandidate`, `sanitizePersistedState`, and all `sanitize*` helpers
- `src/app/state-io.ts` — state load/save operations: `loadState`, `saveState`, `updateState`, `hydrateState`
- `src/app/storage-adapter.ts` — storage adapter factory: `createStorageAdapter`, `StorageOperationResult` helpers
...
- `src/app/replay-utils.ts` — replay reconstruction utilities; exports `reconstructSetupFromRecord(record, runtime)` which converts a history record's stored setup snapshot into a complete non-randomised `GeneratedSetup` object suitable for use with the standard New Game acceptance flow; introduced in Epic 99
- `src/app/setup-rules.ts` — setup template definitions and `resolveSetupTemplate`; resolves player count and play mode into a legal setup template
- `src/app/setup-generator.ts` — setup orchestration only: `generateSetup`, `trySchemeForSetup`, `tryMastermindForScheme`, notice/summary helpers, scheme/mastermind pickers; pool building, freshness, scheme modifiers, validation, hero selection, and category selection extracted into dedicated files below; kept as the public API entry point with backward-compat re-exports (~305 lines)
- `src/app/setup-pool-builder.ts` — builds `GamePool` from runtime data and owned set IDs: `buildOwnedPools`
- `src/app/setup-freshness.ts` — freshness ranking and item selection: `rankItemsByFreshness`, `selectFreshItems`, `shuffle`
- `src/app/setup-scheme-modifiers.ts` — scheme modifier application: `applyModifier`, `applySchemeModifiersToTemplate`
- `src/app/setup-validator.ts` — legality validation: `validateSetupLegality`, `validateBaseCounts`, `validateForcedPickAvailability`, `validateMastermindLeadSlots`
...
- `src/components/App.svelte` — root Svelte 5 component; orchestrates routing and persistence; tab-specific state is delegated to per-tab view-model modules; mounts the `svelte-sonner` `<Toaster>` at `position="bottom-center"` with `expand`, `richColors`, `duration={4000}`, and `theme` driven from the active theme state; `[data-sonner-toaster]` design-token overrides are declared in `app-shell.css` (Epic 69)
- `src/components/TabNav.svelte` — Svelte 5 tab navigation component
- `src/components/BrowseTab.svelte`, `CollectionTab.svelte`, `NewGameTab.svelte`, `HistoryTab.svelte`, `BackupTab.svelte` — Svelte 5 feature-tab components; each owns its tab's reactive template logic, derived state, and top-level composition; large UI sections extracted into sub-components below
- `src/components/GameResultEditor.svelte` — result editor section for `HistoryTab`; receives `resultDraft`, `resultInvalidFields`, and `historyActions` as props
- `src/components/HistoryInsightsDashboard.svelte` — insights dashboard section for `HistoryTab`; receives `dashboard`, `locale`, and `insightsExpanded` as props
- `src/components/ActiveSetFilterPanel.svelte` — active-set filter `<details>` panel for `NewGameTab`; receives `appState`, `gameActions`, `locale`, and `filterFeasibility` as props
- `src/components/ForcedPicksPanel.svelte` — forced-picks `<details>` panel for `NewGameTab`; receives `forcedPicks`, `ownedForcedPickOptions`, `gameActions`, `locale`, and `modeIneligibleSchemeIds` as props
- `src/components/SetupResultCard.svelte` — setup result display for `NewGameTab`; receives `currentSetup`, `soloRulesItems`, and `locale` as props
- `src/components/AboutPanel.svelte` — diagnostics/about panel for `BrowseTab`; receives `bundle`, `appState`, `persistence`, and `locale` as props
- `src/components/CollectionSetsView.svelte` — sets-view section for `CollectionTab`; receives `groupedSets`, `ownedSetSet`, `collectionActions`, and `locale` as props
- `src/components/CardBrowserByCategory.svelte` — renders the card-browser "by category" view inside `CollectionTab`; accepts `pools` (owned-expansion pools object) and `locale` props; calls `getCardsByCategory` and renders one `<details class="history-group">` per non-empty category with a `<summary>` heading and alphabetically sorted card list; all category groups default to open; shows an empty-collection message when no sets are owned (Epics 44, 84, 89)
- `src/components/CardBrowserByExpansion.svelte` — renders the card-browser "by expansion" view inside `CollectionTab`; accepts `pools` and `locale` props; calls `getCardsByExpansion` and renders one `<details class="history-group">` per owned expansion with a `<summary>` containing the expansion name and a `<span class="pill">` count badge, matching the category view's markup pattern; a `data-expansion` attribute on each `<details>` element preserves existing e2e selectors; all expansion groups default to open; shows the same empty-collection message when no sets are owned (Epics 44, 89)
