...
- By Category grouping renders five collapsible `<details class="history-group">` sections in canonical order (Heroes, Masterminds, Villain Groups, Henchman Groups, Schemes), all expanded by default; each section appears only when the owned-expansion pool contains at least one card of that category; cards are listed A–Z by name within each section (Epic 89)
- By Expansion grouping renders one collapsible `<details class="history-group">` section per owned expansion sorted A–Z by expansion name, all expanded by default; each section’s `<summary>` contains the expansion name and a pill count badge showing the total number of cards in that expansion; all cards from that expansion regardless of category are listed A–Z by card name within each section (Epic 89)
...
```
  ...
  │                                                 │
  │  ▶ Active Expansions  (disclosure, collapsed)    │
  │                                                 │
...
- An "Active Expansions" panel (`<details data-active-filter-panel>`) appears below the Forced Picks disclosure on the New Game tab; the panel uses the native `<details>` / `<summary>` collapsible pattern (matching Forced Picks), is collapsed by default on every page load, and can be expanded by clicking the `<summary>` header; the expanded/collapsed state persists for the current session (tab navigation does not reset it, since the component is never unmounted) but resets to collapsed on page reload; once expanded, the panel lists every owned expansion as a toggleable checkbox item; toggling an item adds or removes its ID from `activeSetIds`; "Use all expansions" sets `activeSetIds` to `null` (restoring the all-owned fallback); "Clear selection" sets `activeSetIds` to `[]`, deselecting every expansion checkbox; the panel shows a summary line reading "Using X of Y expansions" when a non-empty filter is active, or "All X expansions" when `activeSetIds` is `null` (Epics 72, 80)
