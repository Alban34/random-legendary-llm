...
### Epic-wide validation gate
- [ ] **Full regression gate:** run `npm test` and `npx playwright test`, and confirm all tests pass before marking Epic 34 work complete

### Story 34.1 — Define the five-mode grouping contract
- [ ] Write the group key derivation rules for all five modes: mastermind (mastermindId), scheme (schemeId), heroes (each heroId), villains (each villainGroupId), player mode (play-mode label)
- [ ] Document that heroes and villains produce one group entry per entity (multi-group membership)
- [ ] Declare mastermind as the default grouping mode, replacing the removed `none` default
- [ ] **Test:** verify the contract document specifies derivation rules for all five modes and multi-group semantics
- [ ] **QC (Automated):** no automated check for documentation; confirm manually before Story 34.2 starts

### Story 34.2 — Add new grouping derivations to `history-utils.mjs`
- [ ] Add `scheme` grouping: derive key from `record.setupSnapshot.schemeId`, resolve display name from the data bundle indexes
- [ ] Add `heroes` grouping: derive one group entry per `heroId` in `setupSnapshot.heroIds`, resolve each display name
- [ ] Add `villains` grouping: derive one group entry per `villainGroupId` in `setupSnapshot.villainGroupIds`, resolve each display name
- [ ] Remove `player-count` from `HISTORY_GROUPING_MODES`
- [ ] Remove `none` from `HISTORY_GROUPING_MODES`
- [ ] Update `DEFAULT_HISTORY_GROUPING_MODE` to remain `'mastermind'`
- [ ] **Test:** verify all five modes produce correct group keys; records with multiple heroes appear in multiple hero groups; records with multiple villain groups appear in multiple villain groups; `player-count` and `none` are no longer valid modes
- [ ] **QC (Automated):** automate QC coverage that exercises all five grouping derivations with representative history records and asserts correct group membership

### Story 34.3 — Update History grouping UI controls
- [ ] Update the grouping control in the History tab to present exactly five mode buttons: Mastermind, Scheme, Heroes, Villains, Player Mode
- [ ] Remove any button or option for `player-count` or `none` from the rendered grouping UI
- [ ] Verify selecting any mode immediately updates the displayed groups
- [ ] **Test:** verify the rendered grouping control exposes exactly five modes and no others; selecting each mode changes the displayed content
- [ ] **QC (Automated):** add browser QC that opens History, verifies five and only five grouping mode controls, and selects each mode to confirm a grouped view renders

### Story 34.4 — Localize all five grouping mode labels
- [ ] Add or update localization keys in English for all five modes: Mastermind, Scheme, Heroes, Villains, Player Mode (`history.groupBy.mastermind`, `history.groupBy.scheme`, `history.groupBy.heroes`, `history.groupBy.villains`, `history.groupBy.playMode`)
- [ ] Add or update the same keys in French
- [ ] Remove or archive localization keys for `player-count` and `none` grouping modes if they existed
- [ ] **Test:** verify all five grouping labels render in English and French without falling back; no untranslated string is visible in either locale
- [ ] **QC (Automated):** add locale-switching QC that opens History in both locales and verifies all five grouping mode buttons display their translated labels

### Story 34.5 — Verify all five grouping modes with existing records
- [ ] Test all five modes against real or representative history records
- [ ] Confirm a record with three heroes appears in all three hero groups
- [ ] Confirm a record with two villain groups appears in both groups
- [ ] Confirm no record is silently omitted from a group it belongs to under any mode
- [ ] **Test:** verify all five modes produce correct groupings for diverse representative setups; multi-entity records appear under all applicable groups
- [ ] **QC (Automated):** run Playwright History tab QC exercising all five grouping modes with seeded data; assert correct group headings and record distribution
