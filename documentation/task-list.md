...
### Epic-wide validation gate
- [ ] **Full regression gate:** run `npm test` and `npx playwright test`, and confirm all tests pass before marking Epic 26 work complete

### Story 26.1 — Audit all base-game, small-expansion, and large-expansion assignments against the published Legendary ruleset

**Status: Approved**

- [ ] Read all set entries in the catalog data source and list their current type assignments
- [ ] Cross-reference each assignment against the published Legendary ruleset definition of base game, small expansion, and large expansion
- [ ] Produce a written correction manifest that names every set whose current classification differs from the target
- [ ] Group the manifest by correction category (base-game, small-expansion, large-expansion) so follow-on stories can consume it directly
- [ ] **Test:** verify the correction manifest accounts for every set in the catalog and that no set is listed under more than one correction category
- [ ] **QC (Automated):** add a QC check that confirms the correction manifest file exists and is non-empty before Story 26.2 work begins

### Story 26.2 — Reclassify Core and Villains as base games throughout the catalog data

**Status: Approved**

- [ ] Locate all catalog data entries for Core and Villains and update their type field to the base-game value
- [ ] Confirm no other set is unintentionally changed by the same edit
- [ ] Verify Core and Villains appear under the base-game group in Browse and Collection surfaces after the change
- [ ] Remove or update any hardcoded label or filter assumption that treated Core or Villains as expansions
- [ ] **Test:** verify Core and Villains render under the base-game category in every surface that groups by set type
- [ ] **QC (Automated):** automate QC coverage that asserts Core and Villains are visible under the base-game group and absent from expansion groups in Browse and Collection

### Story 26.3 — Correct remaining small- and large-expansion misassignments identified by the audit

**Status: Approved**

- [ ] Work through each entry in the Story 26.1 correction manifest that is classified as a small- or large-expansion correction
- [ ] Update each affected catalog entry's type field to the correct target value
- [ ] Confirm no set from the manifest remains uncorrected after changes are applied
- [ ] Verify the corrected sets appear in the expected Browse and Collection groups
- [ ] **Test:** verify every set flagged in the correction manifest now carries the target classification and renders in the correct Browse and Collection group
- [ ] **QC (Automated):** automate QC coverage that iterates the correction manifest and asserts each set appears only under its corrected category in Browse

### Story 26.4 — Fix the Revelations entry to reflect it is a small expansion with no standalone mode

**Status: Approved**

- [ ] Locate the Revelations catalog entry and update its type to small expansion
- [ ] Remove any standalone-mode flag or row for Revelations from the catalog data
- [ ] Confirm no Browse or Collection surface renders a standalone row for Revelations after the change
- [ ] Verify Revelations appears correctly grouped as a small expansion in all affected surfaces
- [ ] **Test:** verify Revelations shows as a small expansion with no standalone entry in Browse and Collection
- [ ] **QC (Automated):** automate QC coverage asserting Revelations is present under small expansions and that no standalone Revelations row appears in any rendered surface

### Story 26.5 — Verify corrected classifications display accurately across Browse and Collection surfaces

**Status: Approved**

- [ ] Run through all corrected sets in Browse and confirm each appears in the expected category group
- [ ] Run through all corrected sets in Collection and confirm each appears in the expected category group
- [ ] Check that alphabetical ordering and filtering show no regressions after the reclassification changes
- [ ] Document any edge cases found during the verification pass and confirm they are resolved or logged
- [ ] **Test:** verify all corrected sets appear in the right Browse and Collection groups with correct alphabetical order and no stale category assignments
- [ ] **QC (Automated):** automate QC coverage for Browse and Collection rendering after the full reclassification, asserting no set appears under an incorrect group
...
### Epic-wide validation gate
- [ ] **Full regression gate:** run `npm test` and `npx playwright test`, and confirm all tests pass before marking Epic 27 work complete

### Story 27.1 — Remove the "Show history-ready setup snapshot" debug panel from all production-rendered surfaces

**Status: Approved**

- [ ] Locate every render path that conditionally or unconditionally shows the "Show history-ready setup snapshot" control or panel
- [ ] Remove the control, trigger button, and panel output from all production-rendered surfaces
- [ ] Confirm the element is absent from the DOM in production builds rather than merely hidden via CSS
- [ ] Verify no other debug surfaces are accidentally removed or left partially visible during the cleanup
- [ ] **Test:** verify the "Show history-ready setup snapshot" button and panel are completely absent from the rendered DOM in production mode across all tabs
- [ ] **QC (Automated):** automate QC coverage asserting the debug snapshot control does not appear in any production-rendered surface

### Story 27.2 — Increase the app title size and align it vertically with the header's language and theme controls

**Status: Approved**

- [ ] Identify the current font-size and vertical-alignment rules applied to the app title element in the header
- [ ] Increase the app title font size to a value that gives the title appropriate visual weight relative to the header controls
- [ ] Apply vertical-alignment rules so the title sits at the same baseline or midpoint as the language selector and theme toggle
- [ ] Verify the alignment holds on both desktop and mobile viewport widths
- [ ] Confirm the updated title does not overflow or wrap unexpectedly on narrow viewports or long locale strings
- [ ] **Test:** verify the app title is visually larger and vertically aligned with the language and theme controls on desktop and mobile
- [ ] **QC (Automated):** automate QC coverage for app title alignment with the language selector and theme toggle across at least two viewport sizes

### Story 27.3 — Verify the revised header remains stable across all themes, locales, and viewport sizes

**Status: Approved**

- [ ] Exercise the revised header in each supported theme and confirm no layout regression or colour clash is introduced
- [ ] Exercise the revised header with each supported locale and confirm titles and labels do not overflow or truncate incorrectly
- [ ] Test the revised header at narrow, medium, and wide viewport widths and confirm the layout remains stable
- [ ] Record any overflow or truncation edge cases found and confirm they are resolved before marking this story done
- [ ] **Test:** verify the revised header renders without overflow, truncation, or layout regression in all supported themes, locales, and viewport sizes
- [ ] **QC (Automated):** automate QC coverage for the revised header across all theme and locale combinations on desktop and mobile viewports
