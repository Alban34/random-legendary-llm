...
import { gotoApp, readAppState, reloadApp, selectTab, setViewport, updateAppState } from './helpers/app-fixture.ts';
...
test.describe('Responsive shell: desktop and mobile layout', () => {
  ...
  test('can traverse the full shell without a mouse and shows visible focus treatment', async ({ page }) => {
    ...
    // 1st Tab → skip link (keyboard-only focus indicator for accessibility)
    await page.keyboard.press('Tab');
    await expect(page.locator('.skip-link')).toBeFocused();
    const skipLinkOutline = await page.evaluate(() => getComputedStyle(document.activeElement).outlineStyle);
    expect(skipLinkOutline).toBe('solid');

    // 2nd Tab → locale select
    await page.keyboard.press('Tab');
