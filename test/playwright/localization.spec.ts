...
test.describe('Localisation: locale coverage', () => {
  test.beforeEach(async ({ page }) => {
    await gotoApp(page);
  });
...
test.describe('Localisation: locale-aware score display', () => {
  test.beforeEach(async ({ page }) => {
    await gotoApp(page);
    await seedAllOwnedState(page);
    await selectTab(page, 'new-game');
...
test.describe('Localisation: version badge', () => {
  test.beforeEach(async ({ page }) => {
    await gotoApp(page);
  });

  test('Story 37.4: app version badge displays a version string', async ({ page }) => {
