...
before(async () => {
  [rendererSource, cssSource, postV1TaskList] = await Promise.all([
...
test('Epic 23.2 — renderer uses <details> element for per-category stats panels', () => {
  assert.match(rendererSource, /stats-category-panel/);
  assert.match(rendererSource, /<details\s[^>]*class="stats-category-panel"/);
});

test('Epic 23.2 — renderer uses <summary> with stats-category-summary class', () => {
  assert.match(rendererSource, /<summary\s[^>]*class="stats-category-summary"/);
});

test('Epic 23.2 — renderer emits data-stats-category attribute on each panel', () => {
  assert.match(rendererSource, /data-stats-category="\$\{category\.category\}"/);
});

test('Epic 23.2 — renderer wraps category body in stats-category-body div', () => {
  assert.match(rendererSource, /class="stats-category-body"/);
});
...
test('Epic 23.3 — renderer does NOT contain the groupingNotice technical disclaimer text', () => {
  assert.doesNotMatch(rendererSource, /groupingNotice/);
  assert.doesNotMatch(rendererSource, /Presentation only\. Grouping never changes/);
});

test('Epic 23.3 — renderer does NOT render the groupingNotice span in grouping controls', () => {
  assert.doesNotMatch(rendererSource, /class="muted">\$\{locale\.t\('history\.groupingNotice'\)\}/);
});

test('Epic 23.4 — renderer renders storage status conditionally on !storageAvailable', () => {
  assert.match(rendererSource, /!\s*persistence\.storageAvailable/);
});

test('Epic 23.4 — renderer does NOT unconditionally display the storage available label', () => {
  assert.doesNotMatch(rendererSource, /persistence\.storageAvailable\s*\?\s*locale\.t\('collection\.storage\.available'\)/);
});
