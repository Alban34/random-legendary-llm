...
test('CollectionTab declares onImportBggCollection prop', () => {

  assert.match(collectionTabSource, /collectionActions\.importBggCollection/);
});

test('CollectionTab renders a bgg-username input', () => {

  assert.match(collectionTabSource, /id="bgg-username"/);
});

test('CollectionTab renders an "Import from BGG" button', () => {

  assert.match(collectionTabSource, /Import from BGG/);
});

test('CollectionTab button is disabled when bggUsername is empty or loading', () => {

  assert.match(collectionTabSource, /disabled=\{bggImportStatus === 'loading' \|\| !bggUsername\.trim\(\)\}/);
});

test('CollectionTab button has aria-busy when loading', () => {

  assert.match(collectionTabSource, /aria-busy=\{bggImportStatus === 'loading'/);
});

test('CollectionTab form prevents default on submit', () => {
...
test('fetchBggCollection returns correct game names for a 200 response with valid XML', async () => {
...
test('fetchBggCollection returns names after 202 then 200 on retry', async () => {
...
test('fetchBggCollection returns ok: false after exhausting retries', async () => {
...
test('fetchBggCollection returns ok: false on network throw', async () => {
...
test('fetchBggCollection returns ok: false on 404 with HTTP error message', async () => {
...
test('fetchBggCollection returns ok: true and empty game names for 200 with no items', async () => {
...
test('matchBggNamesToSets resolves alias "Legendary: A Marvel Deck Building Game" → core-set', () => {
...
test('matchBggNamesToSets canonical name match "Dark City" → dark-city', () => {
...
test('matchBggNamesToSets sends unknown name to unmatched', () => {
...
test('matchBggNamesToSets case-insensitively matches "dark city" → dark-city', () => {
...
test('matchBggNamesToSets handles mixed matched and unmatched in one call', () => {
...
test('matchBggNamesToSets preserves original casing in matched.bggName', () => {
...
test('CollectionTab renders summary section when bggImportSummary is non-null', () => {

  assert.match(collectionTabSource, /data-bgg-summary-panel/);
  assert.match(collectionTabSource, /bggImportSummary !== null/);
});

test('CollectionTab summary section is conditionally rendered', () => {

  assert.match(collectionTabSource, /\{#if bggImportSummary !== null\}/);
});

test('CollectionTab summary includes "BGG Import Summary" heading', () => {

  assert.match(collectionTabSource, /BGG Import Summary/);
});

test('CollectionTab summary includes expansion count sentence', () => {

  assert.match(collectionTabSource, /bggImportSummary\.matched\.length.*expansion\(s\) added/s);
});

test('CollectionTab summary includes "Not found in catalog" section for unmatched', () => {
...
test('CollectionTab summary has a Dismiss button calling onDismissBggSummary', () => {

  assert.match(collectionTabSource, /dismiss-bgg-summary/);
  assert.match(collectionTabSource, /collectionActions\.dismissBggSummary/);
});

test('CollectionTab shows "No matching expansions found" when matched is empty', () => {

  assert.match(collectionTabSource, /No matching expansions found/);
});

test('CollectionTab renders inline error below input when bggImportStatus is error', () => {
